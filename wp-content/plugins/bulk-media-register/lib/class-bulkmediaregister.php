...
class BulkMediaRegister {
	...
	public function __construct() {
		...
		add_action( 'bmr_regist', array( $this, 'regist' ), 10, 2 );
	...
	public function search_files( $uid, $search_text ) {
		...
		if ( empty( $bulkmediaregister_settings['searchdir'] ) ) {
			...
			if ( false !== strpos( $bulkmediaregister_settings['searchdir'], $this->upload_dir ) ) {
				/* Before version 1.19 */
				$bulkmediaregister_settings['searchdir'] = null;
				update_user_option( $uid, 'bulkmediaregister', $bulkmediaregister_settings );
				$searchdir = $this->upload_dir;
			} else {
				/* Version 1.19 later */
				$searchdir = str_replace( $this->upload_path, '', $this->upload_dir ) . $bulkmediaregister_settings['searchdir'];
			}
		}
	...
	public function regist( $file, $uid ) {
		...
		update_user_option( $uid, 'bulkmediaregister_output', $bulkmediaregister_output );
	...
	public function bulkmediaregister_update_callback() {
		...
		if ( check_ajax_referer( $action1, 'nonce', false ) ) {
			if ( current_user_can( 'upload_files' ) ) {
				...
				if ( is_file( $file ) ) {
					do_action( 'bmr_regist', $file, $uid );
					$output = get_user_option( 'bulkmediaregister_output', $uid );
					delete_user_option( $uid, 'bulkmediaregister_output' );
					...
					if ( ( 'image' === $output['file_type'] || 'pdf' === strtolower( $output['ext'] ) ) && ! empty( $output['imagethumburls'] ) ) {
						...
						if ( 'video' === $output['file_type'] || 'audio' === $output['file_type'] ) {
