...
class BulkMediaRegisterAdmin {
	...
	public function __construct() {
		...
		if ( ! class_exists( 'TT_BulkMediaRegister_List_Table' ) ) {
			require_once dirname( __FILE__ ) . '/class-tt-bulkmediaregister-list-table.php';
		}
						...
						if ( check_admin_referer( 'bmr_search', 'bulk_media_register_search' ) ) {
							...
							if ( ! empty( $files ) ) {
								...
								wp_localize_script(
									...
									array(
										'count' => count( $files ),
										'file' => json_encode( $files, JSON_UNESCAPED_SLASHES ),
								...
								<div id="bulkmediaregister-register-container">
									...
									<?php submit_button( __( 'Register' ), 'primary', 'bulkmediaregister_ajax_update', true ); ?>
								</div>
								<?php
							} else {
								?>
								<div class="notice notice-warning is-dismissible"><ul><li><?php esc_html_e( 'File doesn&#8217;t exist?' ); ?></li></ul></div>
								<?php
							}
						}
					} else {
		...
		<div class="wrap">
			<div id="selectmediaregister-loading-container">
				<div style="margin: 5px; padding: 5px;">
					...
					submit_button( __( 'Register' ), 'primary', 'selectmediaregister_ajax_update1', true );
					...
					<?php
					submit_button( __( 'Register' ), 'primary', 'selectmediaregister_ajax_update2', true );
	...
	private function credit() {
		...
		<h3><?php sprintf( esc_html_e( 'Please make a donation if you like my work or would like to further the development of this plugin.', 'bulk-media-register' ) ); ?></h3>
