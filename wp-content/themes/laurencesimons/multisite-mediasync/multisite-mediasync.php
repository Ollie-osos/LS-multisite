...
function msms_getBlogs() {
  global $wpdb;
  return $wpdb->get_results('SELECT blog_id FROM ls_blogs');
}
...
class MultisiteMediaSync {
	...
	function addAdminMenu() {
		add_menu_page( __( 'Multisite MediaSync', 'ajax-thumbnail-rebuild' ), __( 'Multisite MediaSync', 'multisite-mediasync'
 ), 'manage_options', 'multisite-mediasync',  'ManagementPage');
	}
...
function plugin_loaded_function () {
  global $MultisiteMediaSync; $MultisiteMediaSync = new MultisiteMediaSync();
}
